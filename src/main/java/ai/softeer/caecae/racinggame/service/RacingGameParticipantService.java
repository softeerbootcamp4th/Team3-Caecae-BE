...
public class RacingGameParticipantService {
    ...
    public RegisterRacingGameResultResponseDto registerOrUpdateRacingGameResult(RegisterRacingGameResultRequestDto req) {
        ...
        Integer userId = userService.findAndRegisterUser(FindAndRegisterUserRequestDto.builder()
                .phone(req.phone())
                .build()).userId();

        // 찾은 userId 로 레이싱게임에 참여했는지 찾기
        Optional<RacingGameParticipant> optionalRacingGameParticipant = racingGameParticipantRepository.findById(userId);

        // 레이싱게임 참여 이력이 존재하지 않음
        if (optionalRacingGameParticipant.isEmpty()) {
            registerRacingGameResult(req);
     ...
     */
    private RacingGameParticipant registerRacingGameResult(RegisterRacingGameResultRequestDto req) {
